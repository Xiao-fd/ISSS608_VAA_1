...
Moving forward, we will compute summary statistics. These will provide us with valuable
insights, such as the average, median, minimum, and maximum transacted prices, which are pivotal for understanding the market's behavior.
...
1.  The top plot reveals that the Central Region has the highest number of
    sales units compared to other regions, suggesting a denser or more
    active real estate market in this area. The other regions show
    significantly fewer units, with the North East Region and West Region
    being the next most active.

2.  The bottom plot breaks down the distribution within more granular areas.
    Here we see that certain areas, such as Marine Parade and Kallang.
:::

```{r echo=FALSE}
library(ggplot2)
# Creating a bar plot with Planning Area on the x-axis and color distinguished by Planning Region
combined_plot <- ggplot(RT_new, aes(x = `Planning Area`, y = `Number of Units`, fill = `Planning Region`)) +
  geom_bar(stat = "identity", position = position_dodge()) +
  facet_grid(`Planning Region` ~ ., scales = "free_y") +
  labs(
    x = "Planning Area",
    y = "Number of Units",
    title = "Distribution of Units by Planning Area within each Planning Region"
  ) +
  theme_minimal() +
  theme(
  ...
  scale_fill_manual(values = c("steelblue", "lightblue", "gray", "darkblue", "blue"))
...
combined_plot <- ggplot(RT_new, aes(x = `Planning Area`, y = `Number of Units`, fill = `Planning Region`)) +
  ...
  scale_fill_manual(values = c("steelblue", "lightblue", "gray", "darkblue", "purple"))
...
1.  The Central Region shows a concentration of sales, especially in the Downtown Core and Marina East, suggesting a high demand for properties in the city’s commercial heart.

2.  The East Region sees a substantial number of transactions in Bedok and Tampines, indicating these areas are key residential zones with a blend of housing options.

3.  In the North East Region, Serangoon and Hougang stand out, hinting at their popularity and potential as residential areas.

4.  The North Region presents a uniform spread of sales, implying a balanced distribution of housing transactions without a dominant area.

5.  The West Region has a modest distribution with a notable presence in Jurong West, suggesting growth potential in this locality.
:::
