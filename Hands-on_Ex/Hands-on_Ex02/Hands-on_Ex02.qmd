...
In this chapter, we will learn several ggplot2 extensions for creating more elegant and effective statistical graphics. Objectives:
...
Code chunk below will be used to check if these packages have been installed and also will load them onto our working R environment
...
We will use a data file called *Exam_data*. It consists of year end examination grades of a cohort of primary 3 students from a local school. It is in csv file format.
...
**Note**: We can use spec() to quickly inspect the column specifications for this data set.
...
Instead of `geom_text()`, we can use `geom_text_repel().` Instead of `geom_label()`, we can use `geom_label_repel()`.
...
ggplot2 comes with eight [built-in themes](https://ggplot2.tidyverse.org/reference/ggtheme.html) : `theme_gray()`, `theme_bw()`, `theme_classic()`, `theme_dark()`, `theme_light()`, `theme_linedraw()`, `theme_minimal()`, and `theme_void()`. the code
...
including where various labels are placed as well as the fonts that are
used.
